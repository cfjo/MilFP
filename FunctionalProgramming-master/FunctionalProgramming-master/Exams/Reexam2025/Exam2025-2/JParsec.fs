...
module TextParser =
    ...
    let pwhitespace = satisfy Char.IsWhiteSpace <?> "whitespace"
    ...
    let run parser inputStr =
        ...
        function
        | Success (a, _) -> Result.Ok a
        | Failure pe  -> Result.Error(sprintf "%s" (pe.ToString()))
    ...
    let eof : Parser<unit> =
        let parseFun input =
            match input |> nextChar |> snd with
            | None -> Success ((), input)
            | Some c ->
                let err = sprintf "Not at end of input: next char is %c" c 
                Failure (ParseError.mkParseError "satisfy" input err)
        
        mkParser parseFun "eof"
