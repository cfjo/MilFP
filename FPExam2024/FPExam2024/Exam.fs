...
(* 1: Transactions *)
    ...
    let rec balance (trs: transactions) = 
        match trs with
        | Empty -> 0
        | Pay (name, amount, trs) -> balance trs - amount
        | Receive (name, amount, trs) -> amount + balance trs
        
    let balanceAcc (trs: transactions) = 
        let rec aux acc trs =
            match trs with
            | Empty -> acc
            | Pay (name, amount, trs) -> aux (acc - amount) trs
            | Receive (name, amount, trs) -> aux (acc + amount) trs
        aux 0 trs
    ...
    let rec participants (trs: transactions) : (Set<string> * Set<string>) =
        ...
        | Empty -> (Set[], Set[])
        | Pay (name, amount, trs) ->
            match participants trs with
            | (paySet, receiveSet) ->
                paySet.Add name, receiveSet 
        | Receive (name, amount, trs) ->
            match participants trs with
            | (paySet, receiveSet) ->
                paySet, receiveSet.Add name
    
    let rec balanceFold (payFolder: 'a -> string -> int -> 'a) (receiveFolder: 'a -> string -> int -> 'a) (acc: 'a) (trs: transactions) =
        match trs with
        | Empty -> acc
...
(*
    let rec collect (trs: transactions) : Map<string, int> =
        match trs with
        | Pay (name, amount, trs) ->
            match collect trs with
            *)
...
(* Question 2.1 *)
    ...
    A: foo takes a char and makes it into an int.
       bar takes a string and turns it into a char list
       baz takes a list of integers, reverses it and concatenates the contents
       into an int, ex [5; 4; 3] becomes 345
    ...
    A: it has to be a char that reprsents a digit (0 - 9)
    
    Q: The function baz only behaves reasonably if certain 
       constraint(s) are met on its argument. 
       What is/are these constraints?
        
    A: The contents of the list must be digits from 0 - 9, 
       in order to achieve the desired effect of the numbers being
       perfectly reversed  *)
