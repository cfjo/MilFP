module Interpreter.Eval
    ...
    let rec arithEval (expr : aexpr) (st : state) =
        match expr with
        | MemRead a ->
        ...
        | Add (a,b) ->
            Result.bind (fun x ->
            Result.bind (fun y ->
            Ok (x + y)) (arithEval b st)) (arithEval a st)
        | Mul (a,b) ->
            Result.bind (fun x ->
            Result.bind (fun y ->
            Ok (x * y)) (arithEval b st)) (arithEval a st)
        | Div (a,b) ->
            Result.bind (fun x ->
            Result.bind (fun y ->
            if y = 0 then Error DivisionByZero else Ok (x / y)) (arithEval b st)) (arithEval a st)
        | _ -> Ok(0)


    let rec boolEval (bexpr : bexpr) (st : state) =
        match bexpr with
        | TT -> Ok true
        | Not a -> Result.bind (fun x -> Ok (not x)) (boolEval a st)
        | Eq (a,b) -> Result.bind (fun x -> Result.bind (fun y -> Ok (x = y)) (arithEval a st)) (arithEval b st)
        | Lt (a,b) -> Result.bind (fun x -> Result.bind (fun y -> Ok (y < x)) (arithEval a st )) (arithEval b st)
        | Conj (a,b) -> Result.bind (fun x -> Result.bind (fun y -> Ok (x && y)) (boolEval a st)) (boolEval b st)
